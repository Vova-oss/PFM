...
public class TransactionDataDAO {
    ...
    public List<GroupCodesOfClient> findGroupsCodeOfClient(Long id) {
        ...
        String sql = "select \n" +
                ...
                "from pfm.transaction_data ptd\n" +
                "join pfm.mcc pm ON pm.code = ptd.mcc_code\n" +
                "where CAST(replace(sum, ',','.') as float8) < 0 \n" +
                "and ptd.user_id = ?\n" +
                "group by pm.group_code, ptd.user_id\n" +
        ...
        try {
            ...
            while (r.next()) {
                ...
                groupCodesOfClient.setClient_id(r.getLong("user_id"));
